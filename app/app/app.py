...
with tab3:
    ...
    if not registros_acum:
        ...
        # ── Excel Mestre Acumulado ─────────────────────────────────────────────
        st.markdown("#### 📊 Excel Mestre Acumulado")
        st.caption("Gera um Excel consolidado com todas as faturas acumuladas nesta sessão.")

        col_xls, _ = st.columns([2, 3])
        with col_xls:
            gerar_xls = st.button("📊 Gerar Excel Mestre Acumulado", type="secondary")
        ...
        if gerar_xls:
            ...
            regs_mt_acum = [r for r in registros_acum if _is_mt_acum(r)]
            regs_bt_acum = [r for r in registros_acum if not _is_mt_acum(r)]

            nome_arquivo_xls = re.sub(r"[^\w\d\-_]", "_", cliente_val or "Acumulado")[:40]
            ...
            with st.spinner("Gerando Excel..."):
                try:
                    ...
                    with tempfile.TemporaryDirectory() as tmp:
                        if regs_mt_acum:
                            p_mt = Path(tmp) / f"MASTER_MT_Acumulado_{nome_arquivo_xls}.xlsx"
                            gerar_excel_mestre(regs_mt_acum, p_mt, modelo="MT")
                            xls_gerados.append(("MT", p_mt.read_bytes(),
                                                f"MASTER_MT_Acumulado_{nome_arquivo_xls}.xlsx"))
                        if regs_bt_acum:
                            p_bt = Path(tmp) / f"MASTER_BT_Acumulado_{nome_arquivo_xls}.xlsx"
                            gerar_excel_mestre(regs_bt_acum, p_bt, modelo="BT")
                            xls_gerados.append(("BT", p_bt.read_bytes(),
                                                f"MASTER_BT_Acumulado_{nome_arquivo_xls}.xlsx"))

                    if xls_gerados:
                        st.success(f"✅ Excel(s) gerado(s): {len(regs_mt_acum)} MT · {len(regs_bt_acum)} BT faturas")
                        for tipo, xls_bytes, fname in xls_gerados:
                            st.download_button(
                                f"⬇️ Baixar Excel {tipo}",
                                xls_bytes,
                                file_name=fname,
                                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                                key=f"dl_xls_acum_{tipo}",
                            )
                    else:
                        st.warning("Nenhum registro para gerar Excel.")
                except Exception as e:
